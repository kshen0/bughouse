...
(function() {
  var A, B, C, COLORS, D, E, F, G, H, board, boardSquares, calculateThreat, canvas, check, checkForCheck, createBoard, createRectangle, dragLock, drawPieces, endSquare, init, isCheckRemoved, isObstructed, moveSuccess, playerColor, resetBoardColor, resetColors, sendMove, serverBaseUrl, sessionId, setSquareColorForImg, socket, squareSize, startSquare, startX, startY, toggleTurn, whitesTurn;
                ...
                return piece.move(startSquare, endSquare, function(isValid) {
                  ...
                  if (!isValid || isObstructed(startSquare, endSquare, board)) {
                    ...
                    if (!isCheckRemoved(startSquare, endSquare)) {
                      colorturn = whitesTurn ? "white" : "black";
  ...
  toggleTurn = function() {
    ...
    calculateThreat();
    resetColors();
    return check = checkForCheck();
  };

  isCheckRemoved = function() {
    var newCheck;
    if (!check) {
  ...
  checkForCheck = function() {
    ...
    for (x = _i = 0; _i <= 7; x = ++_i) {
      for (y = _j = 0; _j <= 7; y = ++_j) {
        if (((_ref = board[x][y].piece) != null ? _ref.name : void 0) === ("" + colorTurn + " king")) {
          kingSq = board[x][y];
        }
      }
    }
  ...
  resetColors = function() {
    ...
    for (x = _i = 0; _i <= 7; x = ++_i) {
      _results.push((function() {
        ...
        _results1 = [];
        for (y = _j = 0; _j <= 7; y = ++_j) {
          _results1.push(setSquareColorForImg(board[x][y].graphic));
        }
        return _results1;
      })());
    }
